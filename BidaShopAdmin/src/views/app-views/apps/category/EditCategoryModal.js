...
const EditCategoryModal = ({
  ...
  const [imagePreviewUrl, setImagePreviewUrl] = useState("https://i.pinimg.com/736x/57/e0/1e/57e01eac92831addf2960bb870055c2c.jpg"); // State để lưu trữ URL hình ảnh
  ...
  const handleFinish = async (values) => {
    ...
    try {
      ...
      if (error.response && error.response.data) {
        message.error(error.response.data.message); // Hiển thị lỗi từ API
        onCancel();
      } else {
        message.error("Đã xảy ra lỗi không xác định!"); // Fallback error
      }
    } finally {
      // Kết thúc loading
      setLoading(false);
