...
export const getAllDrinkFoods = async (page = 1, limit = 10, keyword = "", categoryId = null, facilityId = null) => {
    try {
        const response = await axios.get(`${API_URL}/drink-foods`, {
            params: { keyword, page, limit, categoryId, facilityId },
...
// ✅ Thêm mới đồ uống/thức ăn
export const createDrinkFood = async (name, price, description, categoryId, imageFile) => {
    try {
...
// ✅ Cập nhật đồ uống/thức ăn
export const updateDrinkFood = async (id, name, price, description, categoryId, imageFile) => {
    try {
        const formData = new FormData();
        formData.append("name", name);
        formData.append("price", price);
